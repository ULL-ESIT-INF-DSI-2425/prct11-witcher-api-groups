...
describe('CRUD completo de /merchants', () => {
  test('POST /merchants → 201 crea buen mercader válido', async () => {
    ...
    const res = await request(app)
      ...
      .send(merchant)
      .expect(201);
    expect(res.body).toMatchObject({ name: 'Merch1', location: 'Novigrado', specialty: 'herrero', contact: 'shop@example.com' });
  });

  test('POST /merchants → 400 si falta campo required', async () => {
    const bad = { location: 'Oxenfurt', specialty: 'alquimista', isTraveling: true, inventorySize: 5, reputation: 3, contact: 'contact@ex.com' };
  ...
  test('POST /merchants → 400 si specialty inválido', async () => {
    const bad = { name: 'BadSpec', location: 'Kaer Morhen', specialty: 'panadero', isTraveling: false, inventorySize: 5, reputation: 2, contact: 'badspec@ex.com' };
    const res = await request(app).post('/merchants').send(bad).expect(400);
  ...
  test('POST /merchants → 400 si inventorySize fuera de rango', async () => {
    const bad = { name: 'Inv0', location: 'Velen', specialty: 'mercachifle', isTraveling: false, inventorySize: 0, reputation: 1, contact: 'inv0@ex.com' };
    const res = await request(app).post('/merchants').send(bad).expect(400);
    expect(res.body.errors.inventorySize).toBeDefined();
  });

  test('POST /merchants → 400 si reputation fuera de rango', async () => {
    const bad = { name: 'Rep11', location: 'Toussaint', specialty: 'sastre', isTraveling: false, inventorySize: 10, reputation: 11, contact: 'rep@ex.com' };
    const res = await request(app).post('/merchants').send(bad).expect(400);
    expect(res.body.errors.reputation).toBeDefined();
  });

  test('POST /merchants → 400 si contact inválido', async () => {
    const bad = { name: 'NoEmail', location: 'Vizima', specialty: 'armero', isTraveling: true, inventorySize: 20, reputation: 7, contact: 'invalid-email' };
    const res = await request(app).post('/merchants').send(bad).expect(400);
  ...
  test('GET /merchants → 200 y lista todos', async () => {
    await MerchantModel.create([
      { name: 'Mer1', location: 'Novigrado', specialty: 'sastre', isTraveling: false, inventorySize: 15, reputation: 4, contact: 'mer1@ex.com' },
      { name: 'Mer2', location: 'Oxenfurt', specialty: 'armero', isTraveling: true, inventorySize: 20, reputation: 6, contact: 'mer2@ex.com' },
    ]);
  ...
  test('GET /merchants?name=Mer1 → 200 y filtra por name', async () => {
    await MerchantModel.create({ name: 'FilterMe', location: 'Cintra', specialty: 'mercachifle', isTraveling: false, inventorySize: 8, reputation: 3, contact: 'filter@ex.com' });
    const res = await request(app).get('/merchants').query({ name: 'FilterMe' }).expect(200);
    expect(res.body[0].name).toBe('FilterMe');
  });

  test('GET /merchants?specialty=armero → 200 y filtra por specialty', async () => {
    await MerchantModel.create({ name: 'SpecTest', location: 'Kaer Morhen', specialty: 'armero', isTraveling: true, inventorySize: 12, reputation: 5, contact: 'spec@ex.com' });
    const res = await request(app).get('/merchants').query({ specialty: 'armero' }).expect(200);
  ...
  test('PATCH /merchants?name=… → 400 sin body', async () => {
    await request(app).patch('/merchants').query({ name: 'NoBody' }).expect(400);
  });

  test('PATCH /merchants?name=… → 400 campo no permitido', async () => {
    await MerchantModel.create({ name: 'Valid', location: 'Oxenfurt', specialty: 'sastre', isTraveling: true, inventorySize: 10, reputation: 8, contact: 'valid@ex.com' });
    await request(app)
      .patch('/merchants')
      .query({ name: 'Valid' })
      .send({ foo: 'bar' })
  ...
  test('PATCH /merchants/:id → 400 sin body', async () => {
    const doc = await MerchantModel.create({ name: 'NoBody', location: 'Kaer Morhen', specialty: 'armero', isTraveling: false, inventorySize: 22, reputation: 3, contact: 'nobody@ex.com' });
    await request(app).patch(`/merchants/${doc._id}`).expect(400);
  });

  test('PATCH /merchants/:id → 400 campo no permitido', async () => {
    const doc = await MerchantModel.create({ name: 'NoFoo', location: 'Vizima', specialty: 'herrero', isTraveling: true, inventorySize: 30, reputation: 4, contact: 'nofoo@ex.com' });
    await request(app)
      .patch(`/merchants/${doc._id}`)
      .send({ bar: 'baz' })
      .expect(400);
